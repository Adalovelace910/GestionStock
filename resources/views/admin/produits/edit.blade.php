...
<a href="{{ route('admin.produits.index') }}" class="btn btn-outline-secondary"><i class="bi bi-list me-1"></i>Liste</a>
</div>
<div class="row">
<div class="col-md-6">
...
<textarea name="description" rows="3" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $produit->description) }}</textarea>
@error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
</div>
<div class="mb-3">
<label class="form-label">Catégorie</label>
<select name="categorie_id" class="form-select @error('categorie_id') is-invalid @enderror" required>
<option value="" disabled {{ old('categorie_id', $produit->categorie_id) ? '' : 'selected' }}>-- Choisir une catégorie --</option>
...
<input type="number" name="prix" step="0.01" min="0.01" value="{{ old('prix', $produit->prix) }}" class="form-control @error('prix') is-invalid @enderror" required>
