...
<h5 class="mb-0">Sorties de stock</h5><a href="{{ route($routePrefix.'.sorties.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>Ajouter une sortie</a>
</div>
<div class="card shadow-sm border-0">
<div class="card-body">
@if($sorties->isEmpty())
<p class="text-muted mb-0">Aucune sortie enregistrée pour le moment.</p>
@else
...
<span class="badge bg-primary">{{ $sortie->produit->categorie->nom }}</span>
@else
<span class="text-muted">—</span>
@endif
</td>
<td>{{ (int) $sortie->quantite }}</td>
<td>{{ $sortie->user->name ?? '—' }}</td>
<td class="text-end">
<a href="{{ route($routePrefix.'.sorties.edit', $sortie) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
@if(auth()->user()->role === 'admin')
<form action="{{ route($routePrefix.'.sorties.destroy', $sortie) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette sortie ?')">
@csrf
@method('DELETE')
