...
class StatistiqueController extends Controller
{
    public function index()
    {
        ...
        $stats = [
            ...
            'valeur_matieres_premieres' => MatierePremiere::selectRaw('SUM(quantite * prix) as total')->value('total') ?? 0,
        ];
        ...
        // Répartition du stock par catégorie (pour le diagramme en anneau)
        $repartitionCategories = Category::withSum('produits', 'quantite')
            ->get()
            ->filter(fn ($categorie) => $categorie->produits_sum_quantite > 0);

        // Matières premières : quantité en stock par matière (diagramme)
        $matieresPremieres = MatierePremiere::orderByDesc('quantite')
            ->take(10)
            ->get();
